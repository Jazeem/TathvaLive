package com.tathva.tathva_live.ui;

import java.text.DecimalFormat;

import com.tathva.tathva_live.activity.AugmentedReality;
import com.tathva.tathva_live.activity.Demo;
import com.tathva.tathva_live.camera.CameraModel;
import com.tathva.tathva_live.common.Vector;
import com.tathva.tathva_live.data.ARData;
import com.tathva.tathva_live.data.PhysicalLocation;
import com.tathva.tathva_live.ui.objects.PaintableBox;
import com.tathva.tathva_live.ui.objects.PaintableBoxedText;
import com.tathva.tathva_live.ui.objects.PaintableGps;
import com.tathva.tathva_live.ui.objects.PaintableObject;
import com.tathva.tathva_live.ui.objects.PaintablePoint;
import com.tathva.tathva_live.ui.objects.PaintablePosition;
...
public class Marker implements Comparable<Marker> {
    ...
    private synchronized void updateView() {
       ...
       // System.out.println("Is on Radar"+isOnRadar+AugmentedReality.portrait);
        if (!isOnRadar) return;
       ...
       // Log.w("lrX: "+lrX, "lrY: "+lrY);//,
       // Log.w("ulX: "+ulX, "ulY: "+ulY);
       // Log.w("width: "+cam.getWidth(), "heiht: "+cam.getHeight());
        
       // Log.w("onradar: "+isOnRadar, "");
        ...
        if(!isInView){
        	 if(lrX<-1) up=true;
        	 if(ulX>cam.getWidth()) down=true;
        	 if(lrY<-1) right=true;
        	 if(ulY>cam.getHeight()) left=true;
        }
     //   Log.w("up: "+up,"down: "+down);
       // Log.w("right: "+right,"left: "+left);
        System.out.println(ARData.orientation);
        locationXyzRelativeToCameraView.get(locationArray);
        if (locationArray[2] >= -1f)
        {
        	if(isInView)
        	{
        		if(ulX>cam.getWidth()/2)down=true;
        		else up=true;
        		
        		if(ulY>cam.getHeight()/2)left=true;
        		else right=true;
   ...
   private synchronized void drawArrows(Canvas canvas){
	   ...
	   Bitmap  b;
    	if(up)
    		{
    		 b=ARData.getBitmap(9);
    		 canvas.drawBitmap(b,0,cam.getHeight()/2-b.getHeight()/2, paint);
    		}
    	if(down)
    	{
    		 b=ARData.getBitmap(3);
    		 
    		 canvas.drawBitmap(b,cam.getWidth()-b.getWidth(),cam.getHeight()/2-b.getHeight()/2, paint); 
    	}
    	if(right)
    	{
    		 b=ARData.getBitmap(12);
    		 canvas.drawBitmap(b,cam.getWidth()/2-b.getWidth()/2,0, paint); 
    	}
    	
    	if(left)
    	{
    		 b=ARData.getBitmap(6);
    		 canvas.drawBitmap(b,cam.getWidth()/2-b.getWidth()/2,cam.getHeight()-b.getHeight(), paint); 
    	}
